...
| Time Fields | `start_time`/`end_time` | Fixed to 0, actual times computed off-chain; start time is VoteMeta transaction block time, end time is start plus fixed duration |
...
In principle, the eligible voter list is straightforward: **the list of users who have staked CKB in Nervos DAO**, which already exists on the CKB chain.
...
1. **Participation Data**: For the DAO 1.1 proposal vote, fewer than 500 people participated—a tiny fraction of total stakers
...
Type CodeHash: `0x38716b429cb139405d32ff86a916827862b2fa819916894848d8460da8953afb`
...
The remaining `cellDep[1]` is the VoteMeta dependency, whose outpoint points to the transaction that created VoteMeta.
...
- **End Height**: Corresponding block height (example: 18,861,255)

> **Note**: CKB epoch length is not fixed; requires calculation and trial. Since epoch 13843 has epoch len 1263, convert the fractional part proportionally to 542/1263.
...
Deserialize using Molecule (structure reference: [vote.mol](https://github.com/CCF-DAO1-1/ckb-dao-vote/blob/main/contracts/ckb-dao-vote/molecules/vote.mol#L13)):
...
Therefore, this vote's voter list is the list of addresses at **height 18,800,000** that:
- Have a DID
- Have staked CKB in Nervos DAO
...
#### 8. Query Stake Amounts

For each address in the DID list, query their stake amount in Nervos DAO:
- Exclude addresses with zero stake
- Remaining addresses form the voter list

> **Current Limitation**: Requires support for querying at specific heights. The new solution is building a Nervos DAO indexer.

#### 9. Verify SMT Root Hash
...
#### 10. Query Address Bindings

For each address in the voter list, call the Address Bind Indexer's `by_to_at_height` (query at voting end height):

```bash
curl -vv http://localhost:9533/by_to_at_height/ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwu8lmjcalepgp5k6d4j0mtxwww68v9m6qz0q8ah/18861255
```

Example response:

```json
[{"height":18861255, "tx_index":1, "from":"ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwu8lmjcalepgp5k6d4j0mtxwww68v9m6qz0q8ah"}]
```
...
Query all addresses' weights (stake amounts) at the voting end height.
...
| 3 | Binding Conflict | If a user votes independently while also bound to another address (like C bound to B in the example), B's weight does not include C's weight |
