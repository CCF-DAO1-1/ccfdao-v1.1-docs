---
title: Vote System
description: Enables creation of votes with multiple candidates, time-bound voting periods, and cryptographic voter list verification using Sparse Merkle Trees (SMT).
---

## System Overview
The Vote System provides a secure, on-chain voting mechanism for proposals. It allows users to create votes with multiple candidates, time-bound voting periods, and cryptographic voter list verification using Sparse Merkle Trees (SMT).
...
graph TB
    ...
    subgraph "Voter List Management"
        VoterListJSON["voter_list.json"]
...
| Field | Type | Purpose |
|-------|------|---------|
| `smt_root_hash` | `Byte32Opt` | Root hash of the Sparse Merkle Tree for voter list |
...
sequenceDiagram
    ...
    Admin->>SMT: Provide voter_list.json
...
- `smt_proof`: Cryptographic proof that this lock_script_hash is in the authorized voter list
...
The Sparse Merkle Tree (SMT) tool processes a voter list and generates cryptographic proofs for efficient on-chain verification.

### SMT Architecture

```mermaid
graph LR
    VoterListJSON["voter_list.json<br/>(lock_script_hashes)"]
    SMTBuilder["smt executable"]
    SMTRoot["smt_root_hash<br/>(Byte32)"]
    Proofs["Individual Proofs<br/>(per lock_hash)"]
    
    VoterListJSON --> SMTBuilder
    SMTBuilder --> SMTRoot
    SMTBuilder --> Proofs
...
1. **Input**: Reads `voter_list.json` containing array of authorized lock script hashes
...
For each authorized voter in the voter list, the SMT tool generates a unique membership proof. This proof allows on-chain verification that a given lock_script_hash is in the authorized set without revealing the entire voter list.
...
3. **Privacy-Preserving**: SMT allows voter list verification without revealing full list
