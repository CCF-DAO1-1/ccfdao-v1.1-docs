...
## 目的与范围
投票系统为 CKB 区块链提供了一个去中心化的链上投票机制。该系统支持创建具有多个候选项、限时投票期和使用稀疏默克尔树（SMT）进行密码学投票人列表验证的投票。与地址绑定系统（参见[地址绑定系统](address-binding)）不同，投票系统采用纯 web3 架构，没有中心化后端——所有数据都存储在链上并在链上验证。

本文档涵盖了投票创建流程、用户投票过程、SMT 投票人列表管理以及整个系统中使用的 Molecule 数据结构。关于 Molecule 序列化格式的详细信息，请参见 Molecule 序列化。
...
| 字段 | 类型 | 目的 |
|-------|------|---------|
| `smt_root_hash` | `Byte32Opt` | 投票人列表的稀疏默克尔树根哈希 |
...
- `smt_proof`：证明此 lock_script_hash 在授权投票人列表中的密码学证明
...
稀疏默克尔树（SMT）工具处理投票人列表并生成用于高效链上验证的密码学证明。
...
1. **输入**：读取包含授权锁脚本哈希数组的 `voter_list.json`
2. **处理**：从锁哈希构造稀疏默克尔树
3. **输出**： 
...
对于投票人列表中的每个授权投票者，SMT 工具生成唯一的成员资格证明。此证明允许链上验证给定的 lock_script_hash 在授权集合中，而无需透露整个投票人列表。
