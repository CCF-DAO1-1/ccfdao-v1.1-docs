...
找任意一个参与了投票的用户，查看其钱包交易历史，找到类似 [此交易](https://testnet.explorer.app5.org/transaction/0xb7dab41c637c2ceaf3d05a8f49513c840d4932263c1d480a56896f70ae249f5d) 的投票交易。

特征是：Type CodeHash testnet 为 `0xb140de2d7d1536cfdcb82da7520475edce5785dff90edae9073c1143d88f50c5`
Type CodeHash mainnet 为 `0x38716b429cb139405d32ff86a916827862b2fa819916894848d8460da8953afb
...
```json
{
  ...
  "lock": {
    ...
    "hashType": "type",
    "args": "0xd7af41f049a29b292dcfa20130bbeabf8d4f268e"
  },
  "type": {
    "codeHash": "0xb140de2d7d1536cfdcb82da7520475edce5785dff90edae9073c1143d88f50c5",
    "hashType": "type",
    "args": "0xc4b5934dbd90ad580c8177a59d404131556aabcb"
  }
  "data": "0x02000000"
}
```
...
[
  ...
  {
    "outPoint": {
      "txHash": "0xd91c95b83597fb7bf741fc0b2245c656154b2f021f23416356b8819fc748442f",
      "index": "0x0"
    },
    "depType": "code"
  },
...
```json
{
  "capacity": "0x50775d800",
  "lock": {
    "codeHash": "0x9bd7e06f3ecf4be0f2fcd2188b23f1b9fcc88e5d4b65a8637b17723bbda3cce8",
    "hashType": "type",
    "args": "0xd7af41f049a29b292dcfa20130bbeabf8d4f268e"
  }
  "data": ""
}
```
...
```javascript
VoteMeta {
  smt_root_hash: '0x90f059af5b62d50ea64b8af5c105a7dbabb1e412a4ec0c78ebbe23b83001f539',
  candidates: [ 'Abstain', 'Agree', 'Against' ],
  start_time: 0,
  end_time: 0,
  extra: '0x42cbdb2e26a9d91d0d014507c3537d49e3ccfc52daadc3466a815ea9f3467d2c'
}
```

- `smt_root_hash` 即本次投票的 voter list 组成的 SMT 的根哈希。
- `candidates` 本次投票的选项


#### 4. 确定投票时间范围

- **开始高度**：创建 VoteMeta 交易所在高度（示例：20,554,834）
- **Epoch 表示**：12790 1097/1800
- **结束高度**：testnet 上的示例是开始高度 + 50个高度，对应区块高度（示例：20,554,884）

mainnet 上按照治理规则的计算示例（项目立项投票间隔为7天，42个Epoch）：
- **结束 Epoch**：开始 Epoch + 42 = 12832 1097/1800
- **结束高度**：对应区块高度（示例：20,629,337  Epoch表示 12832 1097/1800）

> **注意**：CKB mainnet 上的 epoch 长度不固定，有时需要将 Epoch index / Epoch length 部分等比转换(除不尽的时候向上取整)
...
```
20,554,834 → 20,550,000
```

因此，本次投票的 voter list 是在 **20,550,000 高度** 时更新的。

> **边界情况处理**：如果投票开始高度刚刚超过 10,000 的倍数一点点（如 20,550,002），由于上链需要时间，有可能实际使用的是前一个更新高度（20,540,000）。验证时应两个高度 (20,540,000 / 20,550,000) 都应该尝试，继续执行步骤 7-10。
...
```
testnet
https://did-indexer.bbsfans.dev/did-set?until_height=20550000

(mainnet url https://did-indexer.bbs.fans/did-set?until_height=)
```
...
```bash
curl -vv http://localhost:9533/by_to_at_height/ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwh4aqlqjdznv5jmnazqycth64l348jdrscjrd2r/20550000
```

响应示例：

```json
[{"height":20550000, "tx_index":1, "from":"ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwh4aqlqjdznv5jmnazqycth64l348jdrscjrd2r"}]
```
...
```bash
curl -vv http://localhost:9533/query_dao_stake_until_height?until_height=20550000&ckb_list=ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwh4aqlqjdznv5jmnazqycth64l348jdrscjrd2r
```
...
```json
{
  ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwh4aqlqjdznv5jmnazqycth64l348jdrscjrd2r: 200000
}
```
...
```bash
curl -vv http://localhost:9533/by_to_at_height/ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwh4aqlqjdznv5jmnazqycth64l348jdrscjrd2r/20554884
```

响应示例：

```json
[{"height":20554884, "tx_index":1, "from":"ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwh4aqlqjdznv5jmnazqycth64l348jdrscjrd2r"}]
```
...
```bash
curl -vv http://localhost:9533/query_dao_stake_until_height?until_height=20554884&ckb_list=ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwh4aqlqjdznv5jmnazqycth64l348jdrscjrd2r
```
...
``` bash
curl -s 'https://app.ccfdao.dev/api/timeline?uri=at:%2F%2Fdid:ckb:vy5cqmdzhmvxewq4bma74cut5nzumuce%2Fapp.dao.proposal%2F3mhwn5qbgqk2q' | jq -c '.data[0].message' | jq -r . | jq .

{
    ...
    [
      {
        "author": {
          ...
          "ckb_addr": "ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwh4aqlqjdznv5jmnazqycth64l348jdrscjrd2r",
          "created": "2026-03-25T17:44:33+08:00",
          "did": "did:ckb:vy5cqmdzhmvxewq4bma74cut5nzumuce",
          "displayName": "jler",
          "handle": "jler.web5.ccfdao.dev"
        },
        "ckb_addr": "ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwh4aqlqjdznv5jmnazqycth64l348jdrscjrd2r",
        "vote_index": 1,
  ...
  "valid_weight_sum": 20000000000000,
  "vote_sum": 1
}
```
