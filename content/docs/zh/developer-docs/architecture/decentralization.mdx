...
| 时间字段 | `start_time`/`end_time` | 固定为 0，实际时间由链下计算，起始时间为 VoteMeta 交易上链时间，结束时间为起始时间加固定时长 |
...
原则上，有投票权的用户列表非常清晰：**在 Nervos DAO 中质押 CKB 的用户列表**，该列表本身就在 CKB 链上。
...
1. **参与率数据**：以 DAO 1.1 提案投票为例，参与人数不足 500 人，相对于总质押用户仅为极小比例
...
剩余 `cellDep[1]` 即为 VoteMeta 的依赖，其中的outpoint指向创建VoteMeta的交易。
...
- **结束高度**：对应区块高度（示例：18,861,255）

> **注意**：CKB 的 epoch 长度不固定，需要一些计算和尝试。因为epoch 13843的epoch len是1263，分数部分等比转换为 542/1263
...
使用 Molecule 反序列化（结构参见 [vote.mol](https://github.com/CCF-DAO1-1/ckb-dao-vote/blob/main/contracts/ckb-dao-vote/molecules/vote.mol#L13)）：
...
> **边界情况处理**：如果投票开始高度刚刚超过 10,000 的倍数一点点（如 18,807,002），由于上链需要时间，可能实际使用的是前一个更新高度（18,790,000）。验证时应两个高度都尝试。

因此，本次投票的 voter list 是在 **18,800,000 高度**时：
- 拥有 DID
- 且在 Nervos DAO 中质押了 CKB
...
按拥有 DID 的地址列表，逐个查询在 Nervos DAO 中的质押数量：
- 剔除质押为 0 的地址
- 剩余即为 voter list

> **当前限制**：需要支持按高度查询的功能。新方案是建设 Nervos DAO 的 indexer。
...
Voter list 中的每个地址调用 Address Bind Indexer 的 `by_to_at_height`（按投票结束高度查询）：
...
| 1 | 多次投票 | 仅取最后一次 |
| 2 | 多选投票 | 视为无效 |
| 3 | 绑定冲突 | 若用户自身投票同时绑定到另一地址（如示例中的 C 绑定到 B），则 B 的权重不包含 C 的权重 |
